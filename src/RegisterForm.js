...
function RegisterForm() {
  ...
  //create a const to get registered users
...
//fetch users
  ...
  useEffect(() => {
    fetchRegisteredUsers(); // Fetch initial data on component mount
  }, []);
  ...
  return (
    ...
    backgroundRepeat:'no-repeat',backgroundSize:'cover',height:'20cm',borderRadius:'20px'}}>
      ...
      {registrationSuccess && <p style={{color: "white", fontSize: "1.5rem", fontWeight:"bold"}}>Registration successful!</p>} <br/>
      
      <Link to={"/LoginForm"}></Link>
      <button type="login"style={{ marginLeft:"5%",color:'green',borderRadius:'15px',width: "11%"}}> LOG IN </button> 
      
      <button type="submit" style={{ marginLeft:"5%",color:'red',borderRadius:'15px' }}> REGISTER </button>
      </form>
      ...
      <div>
        {registeredUsers.map((user, index) => (
          <div key={index} style={{color:'green', border: '1px solid #ccc', borderRadius: '5px', padding: '10px', marginBottom: '10px' }}>
